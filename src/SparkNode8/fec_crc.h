...
inline void encodeFEC(uint8_t* inBuffer,uint8_t* outBuffer,int numPayload)//numPayload needs to be even
{
	unsigned short beats;
	bool even_beat;
	unsigned short send_packets;
	unsigned short cnt_tx;
	unsigned char last_data;
	bool packet_echo;

    // FEC encode and transmit to FIFO
    unsigned char dataIn[2]; // 2 bytes
    unsigned char dataEnc[4]; // 4 bytes
    unsigned short encCRC;
    unsigned short numBytes;
    unsigned short numRemBytes;
    unsigned short numRemEnc;
    unsigned short numEnc;
	uint8_t data;
    
    fecEncode(NULL, NULL, 0);
    encCRC=0xFFFF;
...
inline bool decodeFEC(uint8_t* inBuffer,uint8_t* outBuffer,int numPayload)//numPayload needs to be even
{
	unsigned short beats;
	bool even_beat;
	unsigned short send_packets;
	...
	bool packet_echo;
	  ...
	  // Query if packets are in from the RX
	  uint8_t num_data, data;
	  unsigned char dataIn[2]; // 2 bytes
	  unsigned char dataEnc[4]; // 4 bytes
	  unsigned char dataOut[4]; // 2 bytes
	  unsigned short numBytes;
	  unsigned short numRemBytes;
	  unsigned short numDec;//, numRemDec;
	  unsigned short decCRC;

	  // This is for BER calculations
	  unsigned char packet_rx[16*2]; // Whole received packet
	  unsigned char *p_rx = packet_rx;
	  unsigned char data_dec[8*2]; // Decoded data
	  unsigned char *p_dec = data_dec;
      ...
      for(int i=0; i<numBytes/2;i++)
      {
	      // Retrieve 4 bytes from FIFO
	      //SPI.transfer(0x3f); dataEnc[0] = SPI.transfer(0x00); //Serial.print(dataEnc[0]); Serial.print(" ");
	      //SPI.transfer(0x3f); dataEnc[1] = SPI.transfer(0x00);//Serial.print(dataEnc[1]); Serial.print(" ");
	      //SPI.transfer(0x3f); dataEnc[2] = SPI.transfer(0x00);//Serial.print(dataEnc[2]); Serial.print(" ");
	     // SPI.transfer(0x3f); dataEnc[3] = SPI.transfer(0x00);//Serial.print(dataEnc[3]); Serial.print(" ");
	      ...
	      {
		      fecDecode(dataOut, dataEnc, numRemBytes);
		      if(numRemBytes > 2)
