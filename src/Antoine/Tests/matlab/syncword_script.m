...
for n = 1:65535
    ...
    total_trans = 0;
    for i = 1:2*N-1
        % Compute integretated sidelobe amplitude
        ISA = ISA + (abs(res(i)));
    end
    for j = 1:N
    ...
    %   [  starts with '1'  ]        [ISA]       [                       DC balance                       ]  [number of transitions]
    %-----------------------------------------------------------------------------------------------------------------------------------------
    %if ((bin_vector(1) == 1) && (ISA <= 44) &&                      (total_ones == 8)                      && (total_trans == 11))
    %if ((bin_vector(1) == 1) && (ISA <= 44) && ((total_ones == 8) || (total_ones == 7) || (total_ones == 9)) && (total_trans == 11)) 
    if ((bin_vector(1) == 1) && (ISA <= 44)  &&                      (total_ones == 8)                      && (total_trans == 10))
    %if ((bin_vector(1) == 1) && (ISA <= 36)  && ((total_ones == 8) || (total_ones == 7) || (total_ones == 9)) && (total_trans == 10))
    %-----------------------------------------------------------------------------------------------------------------------------------------        
        x = x + 1;     
        syncwords(x,1:N) = bin_vector;    
    end   
end

% Display each vectors found
for syncword_number = 1:x
